...
static void OnResultReport(shared_ptr<SessionAsync> ctx, const std::string &bundleName, const std::string &resultInfo)
{
...
static void AdapteCloneOptimize(const string &path)
{
    ...
    if (fd < 0) {
        HILOGE("Failed to open json file = %{private}s, err = %{public}d", manageJsonStr.c_str(), errno);
        return;
    }
    ...
    for (auto name : bundleNames) {
        ...
        for (auto &&bundleInfo : cacheBundleInfos) {
            ...
            bundleInfos.emplace_back(BJsonEntityCaps::BundleInfo {.name = name,
                                                                  ...
                                                                  .fullBackupOnly = bundleInfo.useFullBackupOnly,
