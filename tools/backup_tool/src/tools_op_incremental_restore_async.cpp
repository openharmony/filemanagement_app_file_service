...
static void OnFileReady(shared_ptr<InrementalSessionAsync> ctx, const BFileInfo &fileInfo, UniqueFd fd, UniqueFd rpFd)
{
    ...
    // data path: /data/backup/incrementalreceived/bundleName/{timestamp}/incremental/{filename}
    // For Special clone, timestamp is always 0
    string tmpPath = string(BConstants::BACKUP_TOOL_INCREMENTAL_RECEIVE_DIR) + fileInfo.owner + "/0" +
                     string(BConstants::BACKUP_TOOL_INCREMENTAL) + "/" + formatFileName;
    printf(" OnFileReady tmpPath = %s \n", tmpPath.c_str());
...
static void OnBundleStarted(shared_ptr<InrementalSessionAsync> ctx, ErrCode err, const BundleName name)
{
...
static void OnBundleFinished(shared_ptr<InrementalSessionAsync> ctx, ErrCode err, const BundleName name)
{
...
static void OnAllBundlesFinished(shared_ptr<InrementalSessionAsync> ctx, ErrCode err)
{
    printf("all bundles finished end\n");
    ctx->TryNotify(true);
}

static void OnBackupServiceDied(shared_ptr<InrementalSessionAsync> ctx)
{
...
static void OnResultReport(shared_ptr<InrementalSessionAsync> ctx, const std::string &bundleName,
...
static void RestoreApp(shared_ptr<InrementalSessionAsync> restore, vector<BundleName> &bundleNames)
{
    ...
    for (auto &bundleName : bundleNames) {
        ...
        AdapteCloneOptimize(tmpDataPath);
...
static int32_t AppendBundles(shared_ptr<InrementalSessionAsync> restore,
...
bool IncrementalRestoreAsyncRegister()
{
    return ToolsOp::Register(ToolsOp {ToolsOp::Descriptor {
        .opName = {"incrementalRestoreAsync"},
        .argList = {{
