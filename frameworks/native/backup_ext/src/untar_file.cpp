...
void UntarFile::MatchAregType(bool &isRightRes, FileStatInfo &info, ErrFileInfo &errFileInfo, bool &isFilter)
{
    ...
    if (info.fullPath.length() >= PATH_MAX) {
        HILOGE("File path exceeds PATH_MAX : %{public}s, skip it", GetAnonyPath(info.fullPath).c_str());
        errFileInfo[info.fullPath].emplace_back(ERR_INVALID_PATH);
        if (fseeko(tarFilePtr_, pos_ + tarFileBlockCnt_ * BLOCK_SIZE, SEEK_SET) != 0) {
            HILOGE("Failed to skip file data, err = %{public}d", errno);
            isRightRes = false;
        }
        isFilter = true;
        return;
