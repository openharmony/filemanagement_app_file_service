...
CloneFileInfoBackupRdbstore::CloneFileInfoBackupRdbstore(const std::string &dbPath)
{
    ...
    if (dbPath == "" || !BDir::IsFilePathValid(dbPath) || TryGetRdbStore(dbPath) != NativeRdb::E_OK) {
        HILOGE("Failed to init CloneFileInfoRestore rdb. dbPath: %{public}s", GetAnonyPath(dbPath).c_str());
        return;
    }
