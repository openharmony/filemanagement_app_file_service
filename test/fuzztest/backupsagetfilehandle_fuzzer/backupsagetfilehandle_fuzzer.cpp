...
#include "backupsagetfilehandle_fuzzer.h"
...
bool CmdGetFileHandleFuzzTest(const uint8_t *data, size_t size)
{
    ...
    int len = size >> 1;
    std::string bundleName(string(reinterpret_cast<const char*>(data), len));
    datas.WriteString(bundleName);
    std::string filePath(string(reinterpret_cast<const char*>(data + len), len));
