...
std::string StringUtils::NormalizeLexicalPath(const std::string &filePath)
{
    try {
        return std::filesystem::path(filePath).lexically_normal().string();
    } catch (const std::exception &e) {
        HILOGE("Path normalization failed: %{public}s", e.what());
        return "";
    } catch (...) {
        HILOGE("Path normalization failed: Unknown error");
        return "";
    }
}
} // namespace OHOS::FileManagement::Backup