    ...
    std::map<std::string, bool> &isClearDataFlags)
{
    for (auto &info : backupInfos) {
        ...
        if (BJsonUtil::FindBundleInfoByName(bundleNameDetailMap, bundleNameIndexInfo, UNICAST_TYPE, uniCastInfo)) {
            HILOGI("current bundle:%{public}s, unicast info:%{public}s, unicast info size:%{public}d", bundleNameIndexInfo.c_str(),
                GetAnonyString(uniCastInfo.detail).c_str(), static_cast<uint32_t>(uniCastInfo.detail.size()));
            session_->SetBackupExtInfo(bundleNameIndexInfo, uniCastInfo.detail);
        }
