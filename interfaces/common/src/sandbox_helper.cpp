...
namespace {
    ...
    string deCodeInPutUri = "";
    string deCodeOutPutUri = "";
}
...
{
    if (uri == deCodeInPutUri) {
        return deCodeOutPutUri;
    }
    ...
    while (index < uri.length()) {
        if (uri[index] == '%') {
            int hex = 0;
            std::istringstream inputStream(uri.substr(index + 1, DECODE_LEN));
    ...
    deCodeInPutUri = uri;
    deCodeOutPutUri = outPutStream.str();
    return deCodeOutPutUri;
}
