...
static void ThrowBusinessError(ani_env *env, int errCode, std::string &&errMsg)
{
    ...
    if (ANI_OK != env->Object_SetFieldByName_Double(errorObject, "code", static_cast<ani_double>(errCode))) {
    ...
    if (ANI_OK != env->ThrowError(static_cast<ani_error>(errorObject))) {
        LOGE("throw error object failed");
    }
    return;
}
...
static ani_string GetUriFromPath(ani_env *env, ani_string stringObj)
{
    ...
    ani_status status = env->String_NewUTF8(uri.c_str(), uri.size(), &uriObj);
    if (status != ANI_OK) {
        LOGE("String_NewUTF8 failed with status: %{public}d", status);
        return nullptr;
    }
