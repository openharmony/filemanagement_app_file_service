...
napi_value SessionIncrementalBackupNExporter::GetLocalCapabilities(napi_env env, napi_callback_info cbinfo)
{
...
napi_value SessionIncrementalBackupNExporter::AppendBundles(napi_env env, napi_callback_info cbinfo)
{
    ...
    if (funcArg.GetArgc() == NARG_CNT::ONE) {
        ...
        return NAsyncWorkCallback(env, thisVar, cb, taskName).Schedule(CLASS_NAME, cbExec, cbCompl).val_;
    }
}
...
{
    HILOGI("Called SessionIncrementalBackup::Cancel begin");
    if (!SAUtils::CheckBackupPermission()) {
        HILOGE("Has not permission!");
        NError(E_PERMISSION).ThrowErr(env);
        return nullptr;
...
napi_value SessionIncrementalBackupNExporter::GetCompatibilityInfo(napi_env env, napi_callback_info cbinfo)
{
    HILOGI("Called SessionIncrementalBackupNExporter::GetCompatibilityInfo begin.");
    if (!SAUtils::CheckBackupPermission()) {
        HILOGE("Has not permission!");
        NError(E_PERMISSION).ThrowErr(env);
        return nullptr;
