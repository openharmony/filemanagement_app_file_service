...
#ifndef OHOS_FILEMGMT_BACKUP_B_SESSION_RESTORE_ASYNC_H
#define OHOS_FILEMGMT_BACKUP_B_SESSION_RESTORE_ASYNC_H
...
class BSessionRestoreAsync : public std::enable_shared_from_this<BSessionRestoreAsync> {
public:
    struct Callbacks {
        ...
        std::function<void(const std::string)> onResultReport;
...
public:
     ...
     */
    static std::shared_ptr<BSessionRestoreAsync> Init(Callbacks callbacks);
     ...
     * @return ErrCode 规范错误码
    */
    ErrCode AppendBundlesDetails(UniqueFd remoteCap,
