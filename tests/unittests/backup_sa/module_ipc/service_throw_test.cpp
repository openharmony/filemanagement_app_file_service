...
HWTEST_F(ServiceThrowTest, SUB_Service_throw_InitRestoreSession_0100, testing::ext::TestSize.Level1)
{
    ...
    EXPECT_CALL(*skeleton, GetCallingUid()).WillRepeatedly(Return(BConstants::DEFAULT_USER_ID));
...
HWTEST_F(ServiceThrowTest, SUB_Service_throw_AppFileReady_0100, testing::ext::TestSize.Level1)
{
    ...
    try {
        ...
        EXPECT_CALL(*sessionMock, VerifyBundleName(_, _)).WillOnce(Return(BError(BError::Codes::SA_INVAL_ARG)));
        int fd = 1;
        ...
        EXPECT_CALL(*jsonUtil, BuildBundleNameIndexInfo(_, _)).WillOnce(Return("bundleName"));
        EXPECT_CALL(*sessionMock, VerifyBundleName(_, _)).WillOnce(Return(BError(BError::Codes::SA_REFUSED_ACT)));
