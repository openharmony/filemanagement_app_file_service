...
public:
    ...
    virtual ErrCode VerifyCallerAndGetCallerName(std::string&, bool) = 0;
    ...
    virtual ErrCode AppAncoFileReady(const std::string &, const std::string &, bool) = 0;
...
public:
    ...
    MOCK_METHOD(ErrCode, VerifyCallerAndGetCallerName, (std::string&, bool));
...
{
    return BService::serviceMock->VerifyCallerAndGetCallerName(bundleName, isStrict);
}
...
{
    return BService::serviceMock->AppAncoFileReady(fileName, filePath, needDelete);
}
...
HWTEST_F(ServiceIncrementalTest, SUB_ServiceIncremental_GetIncrementalFileHandle_0100, TestSize.Level1)
{
    ...
    try {
        ...
        EXPECT_CALL(*srvMock, HandleCurBundleFileReady(_, _, _))
            .WillOnce(Return(BError(BError::Codes::OK).GetCode()));
        EXPECT_CALL(*srvMock, GetUserIdDefault()).WillRepeatedly(Return(0));
        auto ret = service->GetIncrementalFileHandle(bundleName, fileName);
            EXPECT_EQ(ret, BError(BError::Codes::OK).GetCode());
