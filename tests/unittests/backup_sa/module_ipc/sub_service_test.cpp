...
HWTEST_F(ServiceTest, SUB_Service_ServiceResultReport_0000, TestSize.Level1)
{
    ...
    try {
        ...
        EXPECT_CALL(*saUtils, IsSABundleName(_)).WillOnce(Return(true));
        EXPECT_CALL(*session, GetServiceReverseProxy()).WillOnce(Return(srProxy));
        EXPECT_CALL(*srProxy, BackupOnResultReport(_, _)).WillOnce(Return(0));
        ...
        EXPECT_CALL(*saUtils, IsSABundleName(_)).WillOnce(Return(true));
        EXPECT_CALL(*session, GetServiceReverseProxy()).WillOnce(Return(srProxy));
        EXPECT_CALL(*srProxy, IncrementalBackupOnResultReport(_, _)).WillOnce(Return(0));
 ...
 * @tc.desc: 测试 ExtConnectDied backupConnection为null且needCleanData为false场景
 * @tc.size: MEDIUM
 * @tc.type: FUNC
 * @tc.level Level 1
 * @tc.require: issueIAKC3I
...
HWTEST_F(ServiceTest, SUB_Service_ExtConnectDied_0000, TestSize.Level1)
{
    ...
    GTEST_LOG_(INFO) << "1. backupConnection is null and needCleanData is false";
...
HWTEST_F(ServiceTest, SUB_Service_ExtConnectDied_0001, TestSize.Level1)
{
    ...
    connectPtr->connState_ = SvcBackupConnection::ConnState::DISCONNECTED;
    EXPECT_CALL(*session, GetExtConnection(_)).WillRepeatedly(Return(wptr(connectPtr)));
    EXPECT_CALL(*connect, GetWasEverConnected()).WillOnce(Return(true)).RetiresOnSaturation();
    EXPECT_CALL(*connect, IsExtAbilityConnected()).WillOnce(Return(false)).RetiresOnSaturation();
    EXPECT_CALL(*session, GetClearDataFlag(_)).WillRepeatedly(Return(false));
    EXPECT_CALL(*session, GetIsRestoreEnd(_)).WillOnce(Return(false));
    EXPECT_CALL(*session, GetScenario()).WillOnce(Return(IServiceReverseType::Scenario::UNDEFINED))
        .WillOnce(Return(IServiceReverseType::Scenario::UNDEFINED));
    EXPECT_CALL(*saUtils, IsSABundleName(_)).WillOnce(Return(true)).RetiresOnSaturation();
    EXPECT_CALL(*session, IsOnAllBundlesFinished()).WillOnce(Return(false));
    EXPECT_CALL(*cdConfig, DeleteClearBundleRecord(_)).WillOnce(Return(true));
    service->ExtConnectDied(callName);
    EXPECT_EQ(service->sched_->bundleTimeVec_.size(), 0);
...
HWTEST_F(ServiceTest, SUB_Service_ExtConnectDied_0002, TestSize.Level1)
{
    ...
    auto connectPtr = sptr(new SvcBackupConnection(callDied, callConnected, callName));
    connectPtr->connState_ = SvcBackupConnection::ConnState::DISCONNECTED;
    EXPECT_CALL(*saUtils, IsSABundleName(_)).WillOnce(Return(true)).WillOnce(Return(true)).RetiresOnSaturation();
    EXPECT_CALL(*session, GetExtConnection(_)).WillRepeatedly(Return(wptr(connectPtr)));
    EXPECT_CALL(*connect, GetWasEverConnected()).WillOnce(Return(true)).RetiresOnSaturation();
...
HWTEST_F(ServiceTest, SUB_Service_ExtConnectDied_0003, TestSize.Level1)
{
    ...
    auto callConnected = [](const string &bundleName) {};
    auto connectPtr = sptr(new SvcBackupConnection(callDied, callConnected, callName));
    connectPtr->connState_ = SvcBackupConnection::ConnState::CONNECTED;
    EXPECT_CALL(*saUtils, IsSABundleName(_)).WillRepeatedly(Return(false));
    EXPECT_CALL(*connect, GetWasEverConnected()).WillOnce(Return(true)).RetiresOnSaturation();
    EXPECT_CALL(*connect, IsExtAbilityConnected()).WillOnce(Return(true)).RetiresOnSaturation();
    EXPECT_CALL(*connect, DisconnectBackupExtAbility()).WillOnce(Return(ERR_OK)).RetiresOnSaturation();
    EXPECT_CALL(*session, GetExtConnection(_)).WillRepeatedly(Return(wptr(connectPtr)));
    EXPECT_CALL(*session, GetClearDataFlag(_)).WillRepeatedly(Return(true));
...
HWTEST_F(ServiceTest, SUB_Service_ReportOnExtConnectFailed_0000, TestSize.Level1)
{
    ...
    try {
        ...
        auto session_ = service->session_;
        service->session_ = nullptr;
        service->ReportOnExtConnectFailed(IServiceReverseType::Scenario::UNDEFINED, bundleName, 0);
        service->session_ = session_;
        EXPECT_TRUE(true);
        ...
        service->ReportOnExtConnectFailed(IServiceReverseType::Scenario::BACKUP, bundleName, 0);
        EXPECT_TRUE(true);
        ...
        service->ReportOnExtConnectFailed(IServiceReverseType::Scenario::RESTORE, bundleName, 0);
        EXPECT_TRUE(true);
...
HWTEST_F(ServiceTest, SUB_Service_OnAllBundlesFinished_0000, TestSize.Level1)
{
    ...
    try {
        ...
        EXPECT_CALL(*param, GetBackupOverrideIncrementalRestore()).WillOnce(Return(false));
        EXPECT_CALL(*session, GetServiceReverseProxy()).WillOnce(Return(srProxy));
        EXPECT_CALL(*srProxy, RestoreOnAllBundlesFinished(_)).WillOnce(Return(0));
        EXPECT_CALL(*param, GetBackupOverrideBackupSARelease()).WillOnce(Return(true));
        service->OnAllBundlesFinished(0);
        EXPECT_TRUE(true);
        ...
        EXPECT_CALL(*session, GetIsIncrementalBackup()).WillOnce(Return(true));
        EXPECT_CALL(*session, GetServiceReverseProxy()).WillOnce(Return(srProxy));
        EXPECT_CALL(*srProxy, IncrementalBackupOnAllBundlesFinished(_)).WillOnce(Return(0));
        EXPECT_CALL(*param, GetBackupOverrideBackupSARelease()).WillOnce(Return(true));
        service->OnAllBundlesFinished(0);
        EXPECT_TRUE(true);
    } catch (...) {
...
HWTEST_F(ServiceTest, SUB_Service_OnAllBundlesFinished_0100, TestSize.Level1)
{
    ...
    try {
        ...
        EXPECT_CALL(*session, GetIsIncrementalBackup()).WillOnce(Return(false));
        EXPECT_CALL(*session, GetServiceReverseProxy()).WillOnce(Return(srProxy));
        EXPECT_CALL(*srProxy, BackupOnAllBundlesFinished(_)).WillOnce(Return(0));
        EXPECT_CALL(*param, GetBackupOverrideBackupSARelease()).WillOnce(Return(true));
        service->OnAllBundlesFinished(0);
        EXPECT_TRUE(true);
        ...
        EXPECT_CALL(*session, ValidRestoreDataType(_)).WillOnce(Return(true));
        EXPECT_CALL(*session, GetServiceReverseProxy()).WillOnce(Return(srProxy));
        EXPECT_CALL(*srProxy, IncrementalRestoreOnAllBundlesFinished(_)).WillOnce(Return(0));
        EXPECT_CALL(*param, GetBackupOverrideBackupSARelease()).WillOnce(Return(true));
        service->OnAllBundlesFinished(0);
        EXPECT_TRUE(true);
...
HWTEST_F(ServiceTest, SUB_Service_TimeoutRadarReport_0000, TestSize.Level1)
{
    ...
    try {
        string bundleName;
        service->TimeoutRadarReport(IServiceReverseType::Scenario::BACKUP, bundleName);
        EXPECT_TRUE(true);

        service->TimeoutRadarReport(IServiceReverseType::Scenario::RESTORE, bundleName);
        EXPECT_TRUE(true);

        service->TimeoutRadarReport(IServiceReverseType::Scenario::UNDEFINED, bundleName);
        EXPECT_TRUE(true);
    } catch (...) {
        EXPECT_TRUE(false);
...
HWTEST_F(ServiceTest, SUB_Service_SendScannedInfo_0000, TestSize.Level1)
{
    ...
    try {
        ...
        EXPECT_CALL(*session, GetScenario()).WillOnce(Return(IServiceReverseType::Scenario::UNDEFINED));
        EXPECT_CALL(*session, GetServiceReverseProxy()).WillOnce(Return(srProxy));
        EXPECT_CALL(*srProxy, BackupOnScanningInfo(_)).WillOnce(Return(0));
        EXPECT_CALL(*task, AddTask(_)).WillOnce(WithArgs<0>(Invoke([](const ThreadPool::Task &f) {
            f();
        })));
...
HWTEST_F(ServiceTest, SUB_Service_GetCompatibilityInfo_0200, testing::ext::TestSize.Level1)
{
    ...
    try {
        ...
        EXPECT_CALL(*svcProxy, HandleGetCompatibilityInfo(_, _, _, _))
