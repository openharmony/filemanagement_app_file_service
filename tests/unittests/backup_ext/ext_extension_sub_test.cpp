...
/**
 * @tc.number: Ext_Extension_Sub_CheckRstoreFileInfos_Test_0300
 * @tc.name: Ext_Extension_Sub_CheckRstoreFileInfos_Test_0300
 * @tc.desc: 测试CheckRstoreFileInfos
 * @tc.size: MEDIUM
 * @tc.type: FUNC
 * @tc.level Level 1
 * @tc.require: I9P3Y3
 */
HWTEST_F(ExtExtensionSubTest, Ext_Extension_Sub_CheckRstoreFileInfos_Test_0300, testing::ext::TestSize.Level1)
{
    GTEST_LOG_(INFO) << "ExtExtensionSubTest-begin Ext_Extension_Sub_CheckRstoreFileInfos_Test_0300";
    ASSERT_TRUE(extExtension != nullptr);
    tuple<bool, vector<string>> result;
    std::string ancoPath = BConstants::PATH_PUBLIC_HOME + BConstants::FUSE_ANCO_DIR;
    extExtension->endFileInfos_.clear();
    extExtension->errFileInfos_.clear();
    extExtension->endFileInfos_[ancoPath + "/test.txt"] = 0;
    result = extExtension->CheckRestoreFileInfos();
    EXPECT_TRUE(std::get<0>(result));

    extExtension->endFileInfos_.clear();
    extExtension->errFileInfos_.clear();
    extExtension->endFileInfos_["../test.txt"] = 0;
    result = extExtension->CheckRestoreFileInfos();
    EXPECT_TRUE(std::get<0>(result));
    GTEST_LOG_(INFO) << "ExtExtensionSubTest-end Ext_Extension_Sub_CheckRstoreFileInfos_Test_0300";
}
...
HWTEST_F(ExtExtensionSubTest, Ext_Extension_Sub_OnBackupExCallback_Test_0100, testing::ext::TestSize.Level1)
{
    ...
    try {
        ...
        errMsg = "test";
        string usrConfig = "{\"allowToBackupRestore\":true}";
        EXPECT_CALL(*extBackupMock, GetUsrConfig()).WillRepeatedly(Return(usrConfig));
        extExtension->OnBackupExCallback(obj)(errCode, errMsg);
        EXPECT_EQ(errMsg, "test");
...
/**
 * @tc.number: SUB_Ext_Extension_Sub_FDSan_DupFd_0201
 * @tc.name: Ext_Extension_Sub_FDSan_DupFd_Test_0201
 * @tc.desc: 测试 dup 后的文件描述符的 FDSan 管理
 * @tc.size: MEDIUM
 * @tc.type: FUNC
 * @tc.level Level 2
 * @tc.require: I9P3Y3
 */
HWTEST_F(ExtExtensionSubTest, Ext_Extension_Sub_FDSan_DupFd_Test_0201, testing::ext::TestSize.Level2)
{
    GTEST_LOG_(INFO) << "ExtExtensionSubTest-begin Ext_Extension_Sub_FDSan_DupFd_Test_0201";
    try {
        string incrementalFile = PATH + BUNDLE_NAME + "incr_dup.txt";
        string manifestFile = PATH + BUNDLE_NAME + "manifest_dup.txt";
        ...
        ASSERT_GE(incrementalFdDup, 0) << "Failed to dup incremental fd";

        int manifestFdDup = dup(manifestFd);
        ASSERT_GE(manifestFdDup, 0) << "Failed to dup manifest fd";

        const char* testData = "FDSan test data";
        write(incrementalFdDup, testData, strlen(testData));
        write(manifestFdDup, testData, strlen(testData));
        ...
        EXPECT_TRUE(false);
        GTEST_LOG_(INFO) << "ExtExtensionSubTest-exception occurred in DupFd FDSan test.";
    }
