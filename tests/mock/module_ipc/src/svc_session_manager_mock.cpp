...
std::tuple<bool, std::map<BundleName, BackupExtInfo>::iterator> SvcSessionManager::GetBackupExtNameMap(const string &)
{
    return {};
}
...
{
    return BSvcSessionManager::sessionManager->GetSessionCallerName();
}

std::string SvcSessionManager::GetSessionActiveTime()
{
    return BSvcSessionManager::sessionManager->GetSessionActiveTime();
}
...
{
    return BSvcSessionManager::sessionManager->GetBackupScene(bundleName);
}
