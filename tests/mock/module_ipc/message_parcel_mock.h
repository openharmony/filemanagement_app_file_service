 ...
 */
#ifndef OHOS_FILEMGMT_BACKUP_MESSAGE_PARCEL_MOCK_H
#define OHOS_FILEMGMT_BACKUP_MESSAGE_PARCEL_MOCK_H
...
class BackupIfaceCast {
public:
    virtual ~BackupIfaceCast() = default;
    virtual sptr<IRemoteBroker> iface_cast(const sptr<IRemoteObject> &) = 0;
public:
    static inline std::shared_ptr<BackupIfaceCast> cast = nullptr;
};

class IfaceCastMock : public BackupIfaceCast {
public:
    MOCK_METHOD1(iface_cast, sptr<IRemoteBroker>(const sptr<IRemoteObject> &));
};

class BackupMessageParcel {
public:
    virtual ~BackupMessageParcel() = default;
public:
...
public:
    static inline std::shared_ptr<BackupMessageParcel> messageParcel = nullptr;
};

class MessageParcelMock : public BackupMessageParcel {
public:
...
#endif // OHOS_FILEMGMT_BACKUP_MESSAGE_PARCEL_MOCK_H