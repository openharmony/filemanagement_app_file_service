...
ErrCode ServiceReverseProxy::BackupOnFileReadysWithoutFd(
    const std::string &bundleName, const BStringRawData &fileNames, const std::vector<int32_t> &errCodes)
{
    return BError(BError::Codes::OK);
}
 
ErrCode ServiceReverseProxy::BackupOnFileReadys(const std::string &bundleName,
    const BStringRawData &fileNames, const std::vector<int> &fds, const std::vector<int32_t> &errCodes)
{
    return BError(BError::Codes::OK);
}
} // namespace OHOS::FileManagement::Backup
